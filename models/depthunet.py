...
class DepthEstimate():
    ...
    def get_trustworthy_depth(self, include_mask=None, confidence=0.9, max_conf_int_width_prop=0.30, include_mask_prop=1.0):
        ...
        accept_mask_noobj = spread < (self.expectation() * max_conf_int_width_prop)

        if include_mask is not None:   
            accept_mask_obj   = spread < (self.expectation() * include_mask_prop)
            ...
            accept_mask = accept_mask_noobj

        est_depth = self.mle()
        trustworthy_depth = est_depth * accept_mask.float()
        return trustworthy_depth
    
    def get_depth_and_trustworthy(self, confidence=0.9, max_conf_int_width_prop=0.30):
        conf_int_lower = self.percentile((1 - confidence) / 2)
        conf_int_upper = self.percentile(1 - (1 - confidence) / 2)

        spread = conf_int_upper - conf_int_lower
        
        accept_mask = spread < (self.expectation() * max_conf_int_width_prop)
        est_depth = self.mle()
        accept_mask = accept_mask.float()
        return est_depth, accept_mask
...
class SimpleUNET(torch.nn.Module):
    def __init__(self, args):
        ...
        self.depth_bins = args.depth_bins
    ...
    def forward(self, rgb, gt_depth = None):
        ...
        bins = self.deconv6(xy1, output_size=rgb.size())

        
        # return bins
        if gt_depth is None:
            depth = bins.argmax(dim=1).float() * (self.max_depth / self.depth_bins)
            return depth
        else:
            b,c,w,h = bins.shape
            gt_depth = ((gt_depth / self.max_depth).clamp(0, 0.999) * self.depth_bins).long()
            gt_depth = gt_depth.flatten()
            bins = bins.permute((0, 2, 3, 1)).reshape([gt_depth.shape[0], self.depth_bins]) 
            loss = torch.nn.functional.cross_entropy(bins, gt_depth, reduction = 'none')
            loss = loss.reshape((b,w,h))
            return loss