...
DEFAULT_RENDER_SETTINGS = {'renderImage': constants.RENDER_IMAGE,
                           'renderDepthImage': constants.RENDER_DEPTH_IMAGE,
                           'renderClassImage': constants.RENDER_CLASS_IMAGE,
                           'renderObjectImage': constants.RENDER_IMAGE,
                           'tempRenderChange' : constants.TEMP_RENDER_CHANGE
                           }
        ...
        if start_unity:
            if platform.system() == 'Linux':
                ...
                if self.docker_enabled:
                    ...
                    if x_display.isnumeric():
                        x_display = ':' + x_display
                    env['DISPLAY'] = x_display
                    print(f'DISPLAY: {x_display}')
    ...
    def step(self, action, smooth_nav=False):
        ...
        if smooth_nav:
            ...
            if "LookUp" in action['action']:
                ...
                action.update(DEFAULT_RENDER_SETTINGS)
                super().step(action)
        ...
        for xx in np.arange(.1, 1.0001, .1):
            if xx < 1:
                teleport_action = {
                    ...
                    'y': position['y'],
                    'horizon': horizon,
                    'tempRenderChange': render_settings['tempRenderChange'],
